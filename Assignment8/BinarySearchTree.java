...
class BinarySearchTree <K extends Comparable<K>, V>  {
	...
	private BSTNode<K,V> insert(BSTNode<K,V> cur, K key, V value){
		if(cur == null){
			count++;
	...
	private V find(K key, BSTNode<K,V> cur) throws KeyNotFoundException{
		if(cur == null){
			throw new KeyNotFoundException();
		}
		if(key.compareTo(cur.key) == 0){
	...
	public List<Entry<K,V>>	entryList() {
		...
		if(root != null){
			q.add(root);
			while(!q.isEmpty()){
				BSTNode<K,V> a = q.removeFirst();
				l.add(new Entry<K,V>(a.key, a.value));
				if(a.left != null){
					q.add(a.left);
				}
				if(a.right != null){
